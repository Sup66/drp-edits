    ...
    if v and user_id ~= nil then -- for each player transforming
      ...
      if tr.units > 0 and recipe then -- check units
        ...
        if money_ok and vipmoney_ok and reagents_ok and inventory_ok then -- do transformation
          ...
          -- VIPMONEY IN!
          if recipe.in_vipmoney > 0 then vRP.tryVipPayment(user_id,recipe.in_vipmoney) end
          for l,w in pairs(recipe.reagents) do
            vRP.tryGetInventoryItem(user_id,l,w,true)
          end
          -- produce products
          ...
          --VIPMONEY OUT!
          if recipe.out_vipmoney > 0 then vRP.giveVipMoney(user_id,recipe.out_vipmoney) end
          for l,w in pairs(recipe.products) do
            vRP.giveInventoryItem(user_id,l,w,true)
          end
